...
func (pb *PowerBar) step(ref int, pbr *PowerBar, snd *Snd) {
	pbval := sys.getChar(ref, 0).getPower()
	power := float32(pbval) / float32(sys.getChar(ref, 0).powerMax)
	level := pbval / 1000
	if pb.levelbars {
		power = float32(pbval)/1000 - MinF(float32(level), float32(sys.getChar(ref, 0).powerMax)/1000-1)
	}
...
func (fa *LifeBarFace) step(ref int, far *LifeBarFace) {
	...
	if sys.getChar(ref, 0) != nil {
		if mg, ok := sys.getChar(ref, 0).anim.remap[group]; ok {
...
func (fa *LifeBarFace) draw(layerno int16, ref int, far *LifeBarFace) {
	if far.face != nil {
		pfx := newPalFX()
		if far.palfxshare {
			pfx = sys.gs.charArray[sys.gs.chars[ref][0]].getPalfx()
		}
		if far.palshare {
			sys.cgi[ref].sff.palList.SwapPalMap(&sys.gs.charArray[sys.gs.chars[ref][0]].getPalfx().remap)
		}
		far.face.Pal = nil
		...
		if !sys.gs.charArray[sys.gs.chars[ref][0]].alive() {
