...
function options.f_saveCfg(reload)
	...
	for k, v in ipairs(options.select_characters) do
		if v["changed"] == true then
			local chara_definition = file_def.rebuild_char(v)
			if v.line == nil then
				local new_line = {kind = "empty", initial_whitespace = ""}
				table.insert(options.select_lines, options.last_character_line, new_line)
				options.last_character_line = options.last_character_line + 1
				v.line = new_line
			end
	...
	if need_to_save_select then
		local select_compiled = file_def.rebuild_source_file(options.select_lines)
		...
		options.should_load_select = true
	end
...
options.should_load_select = true
function options.f_load_select()
	-- load characters data
	local file_def = (loadfile 'external/script/file_def.lua')()
	local section = 0
	local slot = false
	options.select_lines = {}
	options.select_characters = {}
	options.last_character_line = nil
	...
	for select_line in io.lines(motif.files.select) do
		...
		if parsed["kind"] == "section" then
			if parsed["section"]:lower() == "characters" then
				...
				if section == 1 then options.last_character_line = #options.select_lines + 1 end
				section = 0
			end
		elseif section == 1 then
			if parsed["kind"] == "data" then
				...
				elseif slot == false then -- ignore multiple character slot at the moment ;TODO: do not do this
					...
					table.insert(options.select_characters, char_data)
				end
	...
	for k, char_dir in ipairs(listSubDirectory("chars/")) do
		if char_registred_by_folder_name[char_dir:lower()] == nil and char_dir ~= "training" then
			if unused_characted_added == false then
				table.insert(options.select_characters, {special = "marker", display_text = "never included characters"})
	...
	for k, char in ipairs(options.select_characters) do
	...
	options.should_load_select = false
end
...
options.t_itemname = {
	...
	['characters'] = function(cursorPosY, moveTxt, item, t)
		if main.f_input(main.t_players, {'$F', '$B', 'pal', 's'}) then
			if options.should_load_select then
				options.f_load_select()
			end
			local submenu = {}
			submenu["items"] = {}
			submenu["submenu"] = {}
			...
			for k, v in ipairs(options.select_characters) do
				...
				if v["special"] == nil then
					f = function(cursorPosY, moveTxt, item, t)
						if main.f_input(main.t_players, {'$F', '$B', 'pal', 's'}) then
							local char_data = options.select_characters[item]
