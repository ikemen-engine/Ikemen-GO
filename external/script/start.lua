...
function start.f_cellMovement(selX, selY, cmd, side, snd, dir)
	...
	if main.f_input({cmd}, {'$U'}) or dir == 'U' then
		for i = 1, motif.select_info.rows do
			...
			elseif (start.t_grid[selY + 1][selX + 1].char ~= nil or motif.select_info.moveoveremptyboxes == 1 or motif.select_info['cell_' .. col .. '_' .. row .. '_skip'] == 1) and (config.TeamDuplicates or start.t_grid[selY + 1][selX + 1].char == 'randomselect' or not t_reservedChars[side][start.t_grid[selY + 1][selX + 1].char_ref]) and start.t_grid[selY + 1][selX + 1].hidden ~= 2 then
				break
	...
	elseif main.f_input({cmd}, {'$B'}) or dir == 'B' then
		if dir ~= nil then
			...
			for i = 1, motif.select_info.columns do
				...
				if (start.t_grid[selY + 1][selX + 1].char ~= nil or motif.select_info.moveoveremptyboxes == 1 or motif.select_info['cell_' .. col .. '_' .. row .. '_skip'] == 1) and (config.TeamDuplicates or start.t_grid[selY + 1][selX + 1].char == 'randomselect' or not t_reservedChars[side][start.t_grid[selY + 1][selX + 1].char_ref]) and start.t_grid[selY + 1][selX + 1].hidden ~= 2 then
					break
				end
			end
		end
	...
	while not selScreenEnd do
		...
		for row = 1, motif.select_info.rows do
			for col = 1, motif.select_info.columns do
				...
				if (t.char ~= nil and (t.hidden == 0 or t.hidden == 3)) or motif.select_info.showemptyboxes == 1 or motif.select_info['cell_' .. col .. '_' .. row .. '_skip'] == 1 then
					main.f_animPosDraw(
